function [image, info, imrefImage] = ReadMhdImage(filename)
...
ndims = 1;
image = read_raw([ path  filesep  info.DataFile ], info.Dimensions,info.DataType,'native',0,ndims, info);
if (image==-1)
    image = read_raw([   info.DataFile ], info.Dimensions,info.DataType,'native',0,ndims);
end
% Create a matlab imref3d structure:
% Creathe the imref object:
imrefImage = imref3d(size(image), [info.Offset(2) info.Offset(2)+info.PixelDimensions(2)*info.Dimensions(2)], ...
    [info.Offset(1) info.Offset(1)+info.PixelDimensions(1)*info.Dimensions(1)],...
    [info.Offset(3) info.Offset(3)+info.PixelDimensions(3)*info.Dimensions(3)]);
end
